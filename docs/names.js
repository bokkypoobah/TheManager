const Names = {
  ...
  computed: {
    ...
    pagedFilteredSortedAddresses() {
      logInfo("Names", "filteredSortedAddresses - results[0..1]: " + JSON.stringify(this.filteredSortedAddresses.slice(0, 2), null, 2));
  ...
  methods: {
    saveSettings() {
      logInfo("Names", "methods.saveSettings - addressesSettings: " + JSON.stringify(this.settings, null, 2));
      localStorage.addressesSettings = JSON.stringify(this.settings);
    },

    newTransfer(stealthMetaAddress = null) {
      logInfo("Names", "methods.newTransfer - stealthMetaAddress: " + stealthMetaAddress);
    ...
    viewModalAddAccount() {
      logInfo("Names", "methods.viewModalAddAccount BEGIN: " + JSON.stringify(this.settings.newAccount, null, 2));
    ...
    addNewAddress() {
      logInfo("Names", "methods.addNewAddress: " + JSON.stringify(this.newAccount, null, 2));
      this.$refs['modalnewaddress'].hide();
      store.dispatch('data/addNewAddress', this.newAccount);
    },
    addCoinbase() {
      logInfo("Names", "methods.addCoinbase - coinbase: " + this.coinbase);
    ...
    async toggleAddressField(address, field) {
      logInfo("Names", "methods.toggleAddressField - address: " + address + ", field: " + field);
      store.dispatch('data/toggleAddressField', { address, field });
    },
    async setAddressField(address, field, value) {
      logInfo("Names", "methods.setAddressField - address: " + address + ", field: " + field + ", value: " + value);
